...
class EncoderBlock(nn.Module):
    ...
    # src_mask : is the mask we want to apply input 
...
def build_transformer(src_vocab_size:int,target_vocab_size:int,src_seq_len:int,target_seq_len:int,d_model:int=512,N:int=6,h:int=8,dropout:float=0.1,d_ff:int=2048)->Transformer:
    ...
    decoder_blocks=[]
    for _ in range(N):
        decoder_self_attetion_block= MultiheadAttention(d_model,h,dropout)
        decoder_cross_attention_block= MultiheadAttention(d_model,h,dropout)
        feed_forward_block= FNN(d_model,d_ff,dropout)
        decoder_block= DecoderBlock(decoder_self_attetion_block,decoder_cross_attention_block,feed_forward_block,dropout)
